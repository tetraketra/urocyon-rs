...
use crate::urocyon::{
    args::Args, context::RequestContextExtension, database::Database, logs::LogManager, server::Server,
};
...
impl ServerBuilder {
    ...
    pub async fn build(self) -> Result<Server, Error> {
        ...
        let logs = LogManager::register(&args).with_context(|| anyhow!("Failed to register logs."))?;
